...
export default function MyMatchesList() {
  ...
  async function getMyMatches() {
      ...
      const myMatches = await fetch(`/petris/players/${player.id}/matches`, {
        method: 'GET',
        headers: {
  ...
  if (matches.length === 0) {
    return (
      <div>
        <h1 className="text-center">  Mis Partidas</h1>
        <div style={{ textAlign: 'center', width: '50%', alignSelf: 'center', marginLeft: '25%', marginRight: '25%', marginTop: '6%' }}>
          Vaya, parece que no tienes ninguna partida jugada,
          juega partidas para ver los resultados. ¡Diviértete!
        </div>
      </div>
    )
  } else {
